...
class Position:
    ...
    last_sl_date: datetime = None  # 最后一次调整sl的日期
    stop_loss_rate: float = 0.08  # 最大止损比例
    price_tick: float = 0.01
    fix_capital: float = 100000

    def __init__(self, fix_capital=100000, stop_loss_rate=0.08, volume: float = 0, price: float = 0, price_tick: float = 0.01, unit_size: int = 100, direction: Direction = Direction.LONG):
    ...
    def update_position(self, trade: TradeData):
        ...
        if trade.offset == Offset.OPEN and trade.direction == Direction.LONG:
            if trade.volume > 0:
                ...
                self.last_sl_date = trade.datetime
    ...
    def do_stop_loss(self):
        return self.stop_loss_rate > 0

    def get_new_sl_price(self, sm: SourceManager, first: bool = False) -> float:
        if self.volume == 0:
            return None
        ...
        if first and self.last_price != 0:
            if (self.stop_loss_price - low) / (self.last_price * self.stop_loss_rate) <= 0.5:
                price_before_adjust = self.stop_loss_price
                new_stop_loss_price = low # - self.price_tick
                logger.info("[POS][SL_Price_Adjust] date: {}, from {:.2f} to {:.2f}".format(sm.last_date.strftime("%Y-%m-%d"), price_before_adjust, new_stop_loss_price))
            ...
            new_stop_loss_price = min(low_back_price, enter_back_price)  # 两个同时满足，不要太快提高止损位
            # stop_loss_price = (low_back_price + enter_back_price) / 2  # 两个同时满足，不要太快提高止损位
            ...
            last_month_low = sm.recent_week_low(4)
            if self.is_price_high_enough(last_month_high, last_month_low, 5):
                new_stop_loss_price = self.accept_drawback_price(last_month_high, last_month_low, 0.618)

            # 更新止损价
            if new_stop_loss_price > self.stop_loss_price:
                logger.info("[POS][SL_Price_Update] date: {}, from {:.2f} to {:.2f}".format(sm.last_date.strftime("%Y-%m-%d"), self.stop_loss_price, new_stop_loss_price))
                # self.stop_loss_price = stop_loss_price
                return new_stop_loss_price #- self.price_tick

        return self.stop_loss_price

    def update_sl_price(self, new_sl_price, today: datetime):
        # 止损价格只能上升，不能下降
        if new_sl_price is not None:
            _, lw, _ = self.last_sl_date.isocalendar()
            _, tw, _ = today.isocalendar()
            # 对于当周的数据，由于周线未定型，允许向下调整止损价；对于非当周数据，只允许向上调整
            if (lw == tw and self.last_price > self.stop_loss_price) or new_sl_price > self.stop_loss_price:
                self.stop_loss_price = new_sl_price
                self.last_sl_date = today

    def set_sl_price(self, stop_loss: float):
        self.stop_loss_price = stop_loss #- self.price_tick
